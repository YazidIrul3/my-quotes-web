...
const QuotesCategoryReligi = () => {
  const [data, setData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [filter, setFilter] = useState("A - Z");

  const fetchQuotesCategoryReligi = async () => {
    try {
      const data = await fetch("http://localhost:8000/api/quotes/Religi");
...
export default QuotesCategoryReligi;
