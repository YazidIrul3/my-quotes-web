...
const QuotesCategoryMotivation = () => {
  const [data, setData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [filter, setFilter] = useState("A - Z");

  const fetchQuotesCategoryMotivation = async () => {
    try {
      const data = await fetch("http://localhost:8000/api/quotes/Motivation");
...
export default QuotesCategoryMotivation;
