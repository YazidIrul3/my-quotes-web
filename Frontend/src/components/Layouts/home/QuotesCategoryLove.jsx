...
const QuotesCategoryLove = () => {
  const [data, setData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [filter, setFilter] = useState("A - Z");

  const fetchQuotesCategoryLove = async () => {
    try {
      const data = await fetch("http://localhost:8000/api/quotes/Love");
...
export default QuotesCategoryLove;
